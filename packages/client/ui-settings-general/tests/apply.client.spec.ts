...
function usageEntry(slots: SlotRegistry) {
  return slots.entries('settings.section').find(e => e.component === UsageSection)
}
...
describe('ui-settings-general apply', () => {
  ...
  it('fills all five seats for declarations before or after apply', async () => {
    ...
    const usage = usageEntry(before.slots)!
    expect(usage.options).toMatchObject({ id: 'usage', order: 20 })
    expect(resolveSlotLabel(usage.options.label)).toBe('使用统计')
