...
import { usageStatsValueSchema } from '../api/usage.schema.ts'
...
export interface IApiClient {
  ...
  usage: {
    stats(payload: RequestPayload<'usage.stats'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'usage.stats'>>>
  }
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'usage.stats': usageStatsValueSchema,
}
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly usage: IApiClient['usage'] = {
    stats: (payload, signal) => this.callUnary('usage.stats', payload, signal),
  }
