...
3. Release 构建（`tauri build`）：PATH 上的系统 dsh（例如 `npm install -g @deepseek-ai/dsh`），保证全局安装的 dsh 是唯一来源，PowerD 不会重复下载第二份；此时应用内升级按钮禁用，改用 `npm install -g @deepseek-ai/dsh@latest` 升级。
