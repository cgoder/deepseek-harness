...
3. Release builds (`tauri build`): a system-wide dsh on PATH (e.g. `npm install -g @deepseek-ai/dsh`), so a globally installed dsh stays the single source of truth and PowerD never downloads a second copy. The in-app upgrade button is disabled for this source; upgrade with `npm install -g @deepseek-ai/dsh@latest` instead.
